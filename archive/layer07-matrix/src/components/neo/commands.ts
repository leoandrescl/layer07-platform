...
export function toProcesses(projects: Project[]): NeoProcess[] {
...
function matchesProcess(proc: NeoProcess, query: string) {
...
export function completeToken(buffer: string, processes: NeoProcess[]) {
  ...
  switch (cmd) {
    ...
    case "whoami":
      return {
        lines: [
          ...
          out("uid=1000(leonardo)  gid=layer07  tty=neo"),
        ],
      };
    case "stack":
      return {
    ...
    default:
      return {
        lines: [
          echo,
          out(`neo: ${cmd}: command not found  —  try help`, "err"),
        ],
      };
  }
}

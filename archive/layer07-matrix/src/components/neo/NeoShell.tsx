...
export type NeoShellHandle = {
  focus: () => void;
};

type Props = {
  processes: NeoProcess[];
  onFocusChange?: (focused: boolean) => void;
  onClose?: () => void;
  reducedMotion?: boolean;
};

export const NeoShell = forwardRef<NeoShellHandle, Props>(
  function NeoShell({ processes, onFocusChange, onClose, reducedMotion = false }, ref) {
    ...
    const [attached, setAttached] = useState<NeoProcess | null>(null);
    ...
    function attach(proc: NeoProcess) {
    ...
    return (
      <div className="pointer-events-none flex h-full min-h-0 flex-col overflow-hidden border border-dashed border-[#00ff66]/35 bg-black/70">
        ...
        >
          <label htmlFor="neo-tty" className="shrink-0 text-[#00ff66]">
            guest@layer07:~/neo$
          </label>
...
function ProcessInspector({ process }: { process: NeoProcess }) {
  return (

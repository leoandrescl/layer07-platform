...
export function DuvetPlayer() {
  ...
  return (
    <div
      data-no-shot
      data-shot-ui
      className="w-[228px] border border-dashed border-[#00ff66]/40 bg-black/75 px-3 py-2 font-mono text-[#00ff66] backdrop-blur-[2px] sm:w-[248px]"
    >
            ...
            <li key={track.id}>
              <button
                ...
                className={`flex w-full items-baseline gap-2 text-left text-[10px] tracking-[0.14em] uppercase ${
                  selected ? "text-[#00ff66]" : "text-[#64748b] hover:text-[#94a3b8]"
                }`}
              >
                <span className="w-3 shrink-0 text-[9px]">
                  {selected && playing ? ">" : String(index + 1).padStart(2, "0")}
                </span>

...
const HoloHeadline = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
  <span className={`relative inline-block bg-clip-text text-transparent bg-gradient-to-r from-white via-emerald-200/50 to-white bg-[length:200%_auto] animate-shimmer md:whitespace-nowrap ${className}`}>
    {children}
  </span>
);
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        {/* Layer 2: Typography & HUD - Fixed scaling to prevent overflow and ensure mobile hierarchy */}
        <div className="relative z-10 text-center px-6 w-full overflow-visible">
          
          <motion.div style={{ opacity: t1Opacity, y: t1Y }} className="absolute inset-0 flex flex-col items-center justify-center w-full px-4 md:px-10">
            ...
            <div className="mt-8 md:mt-10 bg-black/40 backdrop-blur-md px-6 md:px-8 py-2 border border-emerald-500/20 relative group overflow-visible w-full md:w-auto">
          ...
          <motion.div style={{ opacity: t2Opacity, y: t2Y }} className="absolute inset-0 flex flex-col items-center justify-center w-full px-4 md:px-10">
          ...
          <motion.div style={{ opacity: t3Opacity, y: t3Y }} className="absolute inset-0 flex flex-col items-center justify-center w-full px-4 md:px-10">
               ...
               >
                 <div className="flex items-center gap-4 md:gap-6 bg-emerald-400/5 border border-emerald-400/20 px-6 md:px-10 py-4 md:py-5 backdrop-blur-2xl shadow-[0_0_30px_rgba(16,185,129,0.1)] relative overflow-visible">
                    ...
                    <span className="text-emerald-400 font-mono text-xs md:text-sm tracking-[0.2em] md:tracking-[0.4em] uppercase font-bold">
                      LCP &lt; <NumberTicker value="1" decimals={0} />s <span className="hidden sm:inline text-[10px] ml-2 opacity-50 italic whitespace-nowrap font-normal">AS A STANDARD</span>
                    </span>
                 </div>
               </motion.div>
            </div>
